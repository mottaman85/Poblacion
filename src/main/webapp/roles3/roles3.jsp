...
YPLAD.util.Event.onDOMReady(function() {
	  YPLAD.Alumnos.buscaURL = '<s:url action="roles3" method="buscaRoles" includeParams="none" />';
	  YPLAD.Alumnos.guarda = '<s:url action="roles3" method="guarda" includeParams="none" />';
	  YPLAD.Alumnos.modifica = '<s:url action="roles3" method="modifica" includeParams="none" />';
	  YPLAD.Alumnos.mostrarRE = '<s:url action="roles3" method="mostrarRE" includeParams="none" />';
	  initDtblParametros();
	  initElements();
...
<jsp:include page="roles3-dlg.jsp"></jsp:include>
</html>